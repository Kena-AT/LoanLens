...
class ABTestingFramework:
    ...
    def analyze_results(self, test_id: str, metric: str = "accuracy") -> Dict:
        ...
        return {
            "metric": metric,
            "variant_A": {"n": len(a_values), "mean": mean_a, "std": std_a},
            "variant_B": {"n": len(b_values), "mean": mean_b, "std": std_b},
            "difference": mean_b - mean_a,
            "percent_change": ((mean_b - mean_a) / mean_a * 100) if mean_a != 0 else 0,
            "statistical_test": {"t_statistic": t_stat, "p_value": p_value, "significant": p_value < 0.05},
            "effect_size": {
                "cohens_d": cohens_d,
    ...
    def get_active_tests(self) -> List[str]:
        ...
        return [test_id for test_id, config in self.configs.items() if config.get("status") == "running"]
