    ...
    def _validate_column(self, series: pd.Series, schema: FeatureSchema):
        ...
        if not schema.nullable and series.isnull().any():
            ...
            self.validation_errors.append(
                f"Column '{schema.name}' has {null_count} null values but is non-nullable"
            )
        ...
        if schema.dtype == DataType.NUMERIC:
            if not pd.api.types.is_numeric_dtype(series):
                self.validation_errors.append(
                    f"Column '{schema.name}' should be numeric, got {series.dtype}"
                )
        elif schema.dtype == DataType.INTEGER:
            if not pd.api.types.is_integer_dtype(series):
                self.validation_errors.append(
                    f"Column '{schema.name}' should be integer, got {series.dtype}"
                )
        ...
        if schema.min_value is not None:
            if series.min() < schema.min_value:
                self.validation_errors.append(
                    f"Column '{schema.name}' has values below minimum {schema.min_value}"
                )
        
        if schema.max_value is not None:
            if series.max() > schema.max_value:
                self.validation_errors.append(
                    f"Column '{schema.name}' has values above maximum {schema.max_value}"
                )
